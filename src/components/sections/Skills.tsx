...
const Skills = () => {
  ...
  return (
    <section className="py-14 md:py-20 px-4 relative overflow-hidden bg-muted/30">
      {/* Background Pattern */}
      ...
      >
        ...
        {/* Skills Categories */}
        <motion.div
          className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8 mb-10 md:mb-16"
          variants={containerVariants}
        >
          {skillCategories.map((category, categoryIndex) => (
            <motion.div
              key={categoryIndex}
              className={`relative p-4 md:p-6 rounded-xl md:rounded-2xl border border-border bg-card`}
              variants={itemVariants}
            >
              {/* Category Header */}
              <div className="flex items-center gap-2.5 md:gap-3 mb-4 md:mb-6">
                <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg bg-background/20 border border-border/80 dark:border-white/5 flex items-center justify-center">
              ...
              <div className="space-y-2 md:space-y-3">
                {category.skills.map((skill, skillIndex) => (
                  <motion.div
                    key={skillIndex}
                    className="flex items-center justify-between p-2.5 md:p-3 rounded-lg bg-background/20 backdrop-blur-sm border border-border/80 dark:border-white/5"
                  ...
                  >
                    ...
                    <span
                      className={`px-1.5 py-0.5 md:px-2 md:py-1 rounded-full text-[10px] md:text-xs font-medium border ${getLevelColor(skill.level)}`}
                    >
                      {skill.level}
                    </span>
                  </motion.div>
        ...
        <motion.div className="text-center" variants={itemVariants}>
          ...
          >
            {specialties.map((specialty, index) => (
              <motion.div
                key={index}
                className="px-3 py-1.5 md:px-4 md:py-2 rounded-full bg-primary/10 border border-primary/20 text-primary font-medium text-xs md:text-sm hover:bg-primary/20 transition-colors duration-200"
                variants={skillVariants}
        ...
        {/* Stats */}
        <motion.div
          className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 mt-10 md:mt-16 pt-10 md:pt-16 border-t border-border/50"
          variants={containerVariants}
        >
          {[
            ...
            >
              <motion.div
                className="text-xl md:text-3xl font-bold text-primary mb-1"
                variants={skillVariants}
              >
                {stat.number}
              </motion.div>
